...
- 班長・代表はどちらも「管理者」としてAdmin画面へのフルアクセスを持ちます。設計ドキュメント
  §3 が定める「自分の project_ids の範囲に限定」というスコープ制限はまだ実装していません
  （役職そのものはAdmin → Membersから変更でき、`reports_to_id` による通知先の階層分けは
  実装済みです）。
